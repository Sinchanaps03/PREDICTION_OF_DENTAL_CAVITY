...
def sample_images():
    try:
        image_files = glob.glob('test/*')
        image_files = [img.replace('\\', '/') for img in image_files]  # Normalize path for URLs
        image_files = [img.split('/')[-1] for img in image_files]  # Get filenames only
        return render_template('choose_file.html', sample_images=image_files)
    except Exception as e:
        print(f"Error loading sample images: {e}")
        return render_template('choose_file.html', sample_images=[])

@app.route('/predict_sample/<filename>')
def predict_sample(filename):
    try:
        image_path = os.path.join('test', filename)
        predicted_class, accuracy = predict_image(image_path)
        ...
        return render_template('results.html', status=str_label,status2=f'accuracy is {accuracy}',Treatment=tre,Treatment1=tre1,Recommendation=rec,Recommendation1=Rec1,FollowUp=fl,FollowUp1=Fl1,detection=detection,spread=spread,ImageDisplay=f"/test/{filename}",ImageDisplay1="http://127.0.0.1:5000/static/gray.jpg",ImageDisplay2="http://127.0.0.1:5000/static/edges.jpg",ImageDisplay3="http://127.0.0.1:5000/static/threshold.jpg",ImageDisplay4="http://127.0.0.1:5000/static/sharpened.jpg",ImageDisplay5="http://127.0.0.1:5000/static/matrix.png")
    except Exception as e:
